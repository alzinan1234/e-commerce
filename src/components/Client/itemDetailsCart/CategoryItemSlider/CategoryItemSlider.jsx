...
import "./Products.css";
import { allItems } from "@/app/(home)/page";
import { useState } from "react";
import Link from "next/link";

const CategoryItemSlider = ({}) => {
      ...
      {allItems?.map((item, index) => (
        <SwiperSlide key={index}>
          <div key={index} className="m-1">
            <div
              className="text-center relative mx-auto scale-105  overflow-hidden p-2"
              ...
              >
                {isCustomImage(item.ProductImage) ? (
                  ...
                  <Image
                    ...
                    src={
                      hoveredIndex === index ? item?.image[0] : item?.image[1]
                    }
                    alt=""
                  />
                )}
